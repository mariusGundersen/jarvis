...
export default class Meteogram {
  ...
  async load() {
    ...
    const xml = await fetch('/weather')
      .then(r => r.text())
      .then(text => new DOMParser().parseFromString(text, 'text/xml'));
    this.parseYrData(xml);
  }
  ...
  */
  getTitle(xml) {
    return xml.querySelector('location name').textContent +
      ', ' + xml.querySelector('location country').textContent;
  };
  ...
  */
  createChart(xml) {
    this.chart = new Highcharts.Chart(this.getChartOptions(xml), chart => this.onChartLoad(chart));
  };
  ...
  */
  parseYrData(xml) {
    var forecast = xml && xml.querySelector('forecast');
    var pointStart;
